...
export default {
    tools: {
      ...
      ruler: {
        ...
        longDescription: "オンライン定規は、画面上のオブジェクトを直接測定できる高精度な測定ツールです。クレジットカートなどの標準的なオブジェクトを使用した調整機能を備えており、画面解像度に関係なく正確な測定が可能です。",
        usageContext: "デザイナーや趣味で制作を行う方、または手元に定規がない時に、画面上の小さな要素を素早く測定したい全ての方に最適です。",
        howToUse: "1. cmまたはinchのいずれかの単位を選択します。\n2. クレジットカードなどの標準的なオブジェクトの幅を入力して調整します。\n3. 定規上の任意の場所をダブルクリックして0点を設定します。\n4. 測定したい対象を画面に当てて測定します。",
        faq: [{"q":"画面上の定規はどれくらい正確ですか？","a":"正確さは調整次第です。画面サイズや解像度は様々ですので、物理的なオブジェクト（カード等）を使用した調整が不可欠です。"}]
      },
      wordcounter: {
        title: "文字数カウント",
