...
const TOOL_TEXTS: Record<Locale, { tools: Record<string, ToolText>; common: CommonText }> = {
  ...
  ko: {
    ...
    common: {
      ...
      homeSubtitle: "Apps24는 불필요한 과정 없이 빠르고 간단하며 실용적인 도구를 원하는 사람들을 위해 만들어진 다국어 온라인 도구 웹사이트입니다.",
      homeIntro1: "Apps24는 불필요한 과정 없이 빠르고 간단하며 실용적인 도구를 원하는 사람들을 위해 만들어진 다국어 온라인 도구 웹사이트입니다. 작은 작업을 위해 소프트웨어를 다운로드하거나 계정을 만들 필요 없이, 사용자는 페이지를 열고 즉시 도구를 사용해 몇 번의 클릭만으로 결과를 얻을 수 있습니다.",
      homeIntro2: "우리의 목표는 다양한 사용자들이 일상적인 디지털 작업을 더 쉽게 처리할 수 있도록 돕는 것입니다. 단어 수를 확인하는 학생, 링크나 콘텐츠를 준비하는 마케터, JSON이나 Base64 데이터를 확인하는 개발자, 색상을 다루는 디자이너, QR 코드와 바코드를 만드는 사업자까지, Apps24는 빠르게 접근할 수 있고 쉽게 이해할 수 있는 가벼운 도구를 제공합니다.",
