...
const referenceTitles: Record<Locale, string> = {
  ...
  es: "Referencias", pt: "Referências", ja: "参考資料", zh: "参考来源",
  "zh-TW": "參考來源", ar: "المراجع",
};

const presetTitles: Record<Locale, string> = {
  en: "Quick Setups", ko: "빠른 설정", fr: "Réglages rapides", de: "Schnelleinstellungen",
  es: "Ajustes rápidos", pt: "Configurações rápidas", ja: "クイック設定", zh: "快捷设置",
  "zh-TW": "快速設定", ar: "إعدادات سريعة",
};
...
export async function ToolShell({
  ...
  const longtailSectionTitle = presetTitles[locale];
  const categoryLabel = (() => {
        ...
        <section className="tool-rich-content-section">
          ...
          {toolText.examples && toolText.examples.length > 0 && (
            <div className="content-block" style={{ marginBottom: "2.5rem" }}>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 220px), 1fr))" }}>
          ...
          {highIntentUseCases && !toolText.references && (
          ...
          {(unitConverterLinks.length > 0 || percentageCalculatorLinks.length > 0 || qrGeneratorLinks.length > 0 || imageCompressorLinks.length > 0 || pixelConverterLinks.length > 0 || backgroundRemoverLinks.length > 0 || base64EncoderLinks.length > 0 || jsonFormatterLinks.length > 0 || barcodeGeneratorLinks.length > 0 || popularConversions.length > 0) && (
            <div className="content-block" style={{ marginBottom: "2.5rem" }}>
              ...
              <ul style={{ listStyle: "none", padding: 0, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 250px), 1fr))", gap: "1rem" }}>
          ...
          {toolText.references && (
            <div className="content-block" style={{ marginBottom: "2.5rem" }}>
              <h2>{referenceTitles[locale]}</h2>
              <ul>
                {Object.entries(toolText.references).map(([label, href]) => (
